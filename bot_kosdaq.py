# (위 KOSPI 코드와 동일하며, 수집 URL의 sosok=0만 sosok=1로 바뀐 버전입니다)
# 지수님, 코스닥용은 sosok=1로 설정되어 있으니 그대로 쓰시면 됩니다.
...
async def get_data():
    all_stocks = []
    init_html = fetch_page("https://finance.naver.com/sise/sise_market_sum.naver?sosok=1")
    if not init_html: return pd.DataFrame()
    last_p = min(int(max(re.findall(r'page=(\d+)', init_html), default=1)), 30)
    
    print(f"📡 KOSDAQ 수집 시작 (목표: {last_p}p)...")
    for p in range(1, last_p + 1):
        url = f"https://finance.naver.com/sise/sise_market_sum.naver?sosok=1&field=quant&field=open&field=high&field=low&field=frate&page={p}"
        html = fetch_page(url)

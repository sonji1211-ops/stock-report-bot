...
async def get_kosdaq_market_scan():
    """요구사항 1: 코스닥 전수조사 (BeautifulSoup 정밀 파싱)"""
    ...
    all_stocks = []
    # market_code 1 = KOSDAQ
    headers = {'User-Agent': random.choice(user_agents), 'Referer': 'https://finance.naver.com/'}
    
    # 1. 마지막 페이지 확인
    init_text = fetch_naver_page(f"https://finance.naver.com/sise/sise_market_sum.naver?sosok=1", headers)
    ...
    print(f"📡 KOSDAQ 전수조사 시작 ({last_page}페이지)...")
                ...
                if table:
                    ...
                    if df_list:
                        ...
                        if '시가' in df.columns:
                            ...
                            success = True
                            break
            # 코스닥은 종목이 많으므로 차단 방지를 위해 랜덤 지연을 확실히 줌
...
async def send_kosdaq_report():
    bot = Bot(token=TOKEN)
    now = datetime.utcnow() + timedelta(hours=9)
    
    try:
        df_final = await get_kosdaq_market_scan()
        if df_final.empty:
            print("❌ KOSDAQ 데이터 수집 실패"); return
        ...
        # 통계 메시지 구성 (지수님 요청 사항)
        up_cnt = len(sheets['코스닥_상승'])
        down_cnt = len(sheets['코스닥_하락'])
        msg = (f"📅 {now.strftime('%m-%d')} *[KOSDAQ {report_type}]*\n"
               f"📊 분석: 코스닥 전 종목 ({len(df_final)}개) 전수조사\n"
        ...
        os.remove(file_name)
        print("✅ KOSDAQ 전용 리포트 전송 성공!")

    except Exception as e:
        print(f"❌ KOSDAQ 오류: {e}")

if __name__ == "__main__":
    asyncio.run(send_kosdaq_report())

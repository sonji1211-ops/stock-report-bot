...
def fetch_naver_stock(sosok, page):
    ...
    try:
        # 타임아웃을 15초로 늘려 깃허브 액션 지연 대비
        resp = requests.get(url, headers=headers, timeout=15)
        if resp.status_code != 200: return []
        
        # html.parser로 변경 (lxml 미설치 환경 대비)
...
async def run_report():
    ...
    for p in range(1, 31):
        ...
        if not data:
            print(f"ℹ️ {p}p에서 데이터가 없거나 수집이 중단되었습니다.")
            break
        all_data.extend(data)
        # 매 페이지마다 로그를 찍어 멈춤 현상 확인
        print(f"📑 {p}/30 페이지 수집 완료 (누적 {len(all_data)}개)")
        time.sleep(random.uniform(0.5, 1.2))

    df = pd.DataFrame(all_data)
    if df.empty:
        print("❌ 수집된 데이터가 최종적으로 0개입니다.")
        return

    r_type = "주간평균" if now.weekday() == 6 else "일일"
    file_name = f"{now.strftime('%m%d')}_KOSPI_{r_type}.xlsx"
    
    # 지수님 필터링 요구사항 (5% 이상/이하)
    ...
    with pd.ExcelWriter(file_name, engine='openpyxl') as writer:
        for s_name, d in {'코스피_상승': up_df, '코스피_하락': down_df}.items():
            ...
            for r in range(2, ws.max_row + 1):
                try:
                    val_text = ws.cell(r, 6).value
                    v = abs(float(val_text)) if val_text and str(val_text).replace('.','').isdigit() else 0
